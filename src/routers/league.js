...
router.get('/league/team/add', async (req, res) => {
  ...
  try {
    ...
    if (table.teams.length === 40)
      throw new Error('Table is full, max of 40 teams; cannot add more teams!')
  ...
  await metadata.find({}, async (_e, meta) => {
    ...
    if (table.teams.length < 10)
      return res.status(400).send({
        feedBack:
          'Teams are not up to 10; min of 10 and max of 40 teams take in.',
      })
    table.running = 'yes'
    for (i = 0; i < table.teams.length; i++) {
