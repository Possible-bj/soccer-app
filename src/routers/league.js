...
router.get('/league/team/add', async (req, res) => {
    ...
    try {
        ...
        const table = await league.findOne({ season })
        if (table.running === 'yes') throw new Error('League has Started, cannot add more teams!')
        if (table.running === 'Ended') throw new Error('League has Ended, cannot add teams!')
        if (table.teams.length === 32) throw new Error('Table is full, max of 32 teams; cannot add more teams!')
...
router.get('/league/team/remove', async (req, res) => {
    ...
    try {
        ...
        const season = meta[0].league.season
            const table = await league.findOne({ season })
            if (table.running === 'yes') throw new Error('League has Started, cannot remove teams!')
            if (table.running === 'Ended') throw new Error('League has Ended, cannot remove teams!')
